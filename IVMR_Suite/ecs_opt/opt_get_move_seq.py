...
class MoveSeqGenerator:
    def __init__(self, data_path, is_filter_unmovable=False, is_process_numa=True, logger_path=None, solution=None):
        ...
        self.ecs_env = ECSEnvironment(data_path=self.data_path, is_limited_count=True, 
                                      is_filter_unmovable=self.is_filter_unmovable, is_process_numa=self.is_process_numa)
        self.item_infos = dcp(self.ecs_env.item_full_infos)
        self.box_infos = dcp(self.ecs_env.box_full_infos)
        if self.is_filter_unmovable:
            self.item_infos = self.item_infos[self.ecs_env.item_init_movable]
        self.configs = dcp(self.ecs_env.configs)
        self.init_placement = dcp(self.ecs_env.cur_placement)
        
        self.box_count = len(self.box_infos)
    ...
    def opt_solve_ecs(self, sol_save_path=None, is_solve_cycle=True, is_undo_neg=True, is_sorted_moves=True):
        st = time.time()
        item_count, box_count = len(self.ecs_env.item_cur_state), len(self.ecs_env.box_cur_state)
        opt_sol = np.zeros((item_count, box_count), dtype=int)
        opt_numa_sol = {}
        if not self.solution:
            try:
                s1 = time.time()
                info = f"Load Solution from {sol_save_path}..."
                self._write_info(info)
                if sol_save_path and os.path.exists(sol_save_path):
                    solution = {}
                    variables = ET.parse(sol_save_path).getroot().find('./variables')
                    for item in variables:
                        name, val = item.attrib['name'], float(item.attrib['value'])
                        solution[name] = val
                    for item_i in range(item_count):
                        for box_j in range(box_count):
                            opt_sol[item_i, box_j] = solution[f'x_{item_i}_{box_j}']
                            lri, lrb = self.item_infos.iloc[item_i]['numa'][1], len(self.box_infos.iloc[box_j]['numa'])
                            if (lri > 0) and (lrb > 0) and (lri <= lrb) and (self.box_infos.iloc[box_j]['isInfinite'] <= 0) and (self.item_infos.iloc[item_i]['canMigrate'] >= 1):
                                opt_numa_sol[(item_i, box_j)] = np.zeros(lrb, dtype=int)
                                for rbi in range(lrb):
                                    opt_numa_sol[(item_i, box_j)][rbi] = solution[f"numa_{item_i}_{box_j}_{rbi}"]
                    info = f"Finish Loaded Solution from {sol_save_path}, Elapsed Time {time.time()-s1}s..."
                    self._write_info(info)
                else:
                    info = f"Load Solution Error!"
                    self._write_info(info)
                    return
            except Exception as e:
                info = f"No Solution is Provided!"
                self._write_info(info)
                self._write_info(e)
                return
        else:
            for item_i in range(item_count):
                for box_j in range(box_count):
                    opt_sol[item_i, box_j] = self.solution.get_value(f'x_{item_i}_{box_j}')
                    lri, lrb = self.item_infos.iloc[item_i]['numa'][1], len(self.box_infos.iloc[box_j]['numa'])
                    if (lri > 0) and (lrb > 0) and (lri <= lrb) and (self.box_infos.iloc[box_j]['isInfinite'] <= 0) and (self.item_infos.iloc[item_i]['canMigrate'] >= 1):
                        opt_numa_sol[(item_i, box_j)] = np.zeros(lrb, dtype=int)
                        for rbi in range(lrb):
                            opt_numa_sol[(item_i, box_j)][rbi] = self.solution.get_value(f"numa_{item_i}_{box_j}_{rbi}")
        
        init_score = self.ecs_env.get_cur_state_score()
        init_vio_cost, init_vio_info = self.ecs_env.get_cur_state_vio_cost()
        self.ecs_env.cur_placement = opt_sol
        self.ecs_env._convert_constrs_vio_to_objective()
        opt_score = self.ecs_env.get_cur_state_score(is_contain_migration=True)
        opt_vio_cost, opt_vio_info = self.ecs_env.get_cur_state_vio_cost()
        self.ecs_env.cur_placement = np.array(self.init_placement).copy()
        self.ecs_env._convert_constrs_vio_to_objective()
        st1 = time.time()
    
        reward_matrix = self.ecs_env._get_reward_batch(range(box_count))
        reward_matrix += self.ecs_env._get_vio_cost_batch()
        moves, reward, numa_moves = [], [], []
        for i, j in zip(*np.where(opt_sol != self.init_placement)):
            if opt_sol[i, j]:
                moves.append([i, j])
                reward.append(reward_matrix[i, j])
                if self.ecs_env.len_numa_res > 0:
                    numa_action = np.zeros(self.configs['maxBoxNuma'], dtype=int)
                    opt_numa = opt_numa_sol.get((i, j))
                    if opt_numa is not None:
                        numa_action[np.where(opt_numa > 0)[0]] = 1
                else:
                    numa_action = None
                    # numa_action[np.where(opt_numa_sol[(i, j)] > 0)[1]] = 1
                numa_moves.append(numa_action)
        ...
        if self.ecs_env.len_numa_res > 0:
            ...
            numa_moves = np.array(numa_moves)[sorted_idxes]
        st2 = time.time()
        info = f"Initial Score: {init_score}, Optimal Score: {opt_score}, Optimal Move Count: {len(moves)}! Elapsed Time: {st1-st}s, {st2-st1}s.\n"
        info += f"Initial State Vio Cost: {init_vio_cost}, {init_vio_info}\n"
        info += f"Opt State Vio Cost: {opt_vio_cost}, {opt_vio_info}"
        self._write_info(info)
        # pdb.set_trace()
        ...
        while len(moves) > 0:
            ...
            for i, m in enumerate(moves):
                ...
                if self.ecs_env.is_action_available(list(m), numa_moves[i])[0]:
                    st3 = time.time()
                    ...
                    info += f"SVio {move_count}: {infos['vio_info']}"
                    self._write_info(info)
        ...
        vio_cost_1, vio_info_1 = self.ecs_env.get_cur_state_vio_cost()
        ...
        vio_cost_2, vio_info_2 = self.ecs_env.get_cur_state_vio_cost()
        ...
        info += f"Final Vio Cost: {vio_cost_1}, {vio_info_1}\n"
        info += f"Eval Final Vio Cost: {vio_cost_2}, {vio_info_2}"
        self._write_info(info)
        
        if is_sorted_moves and (len(self.real_moves) > 0):
            tr, fs = self.sort_moves()
            total_reward, costs, invalid_num = self.ecs_env.eval_move_sequence(self.real_moves, self.real_numa_moves)
            vio_cost, vio_info = self.ecs_env.get_cur_state_vio_cost()
            st6 = time.time()
            ...
            info += f"After Sorting Moves, Vio Cost: {vio_cost}, {vio_info}"
            self._write_info(info)
    ...
    def sort_moves(self):
        ...
        for m, nm in zip(self.real_moves, self.real_numa_moves):
            # pre_res = self.ecs_env.box_cur_state[m[1], -len(self.ecs_env.configs["resource_types"]):]
            _, r, d, infos = self.ecs_env.step(list(m), nm)
            # aft_res = self.ecs_env.box_cur_state[m[1], -len(self.ecs_env.configs["resource_types"]):]
            # if (pre_res > 0).all() and (aft_res < 0).any():
            #     pdb.set_trace()
            total_reward += r
            info = f"Sorted Step {infos['move_count']}: {r}, {d}, {infos['action_info']}\n"
            info += f"Sorted SVio {infos['move_count']}: {infos['vio_info']}"
            self._write_info(info)
        final_score = self.ecs_env.get_cur_state_score()
        # pdb.set_trace()
        return total_reward, final_score
    
    def _sort_move_seq(self, move_seq):
        self._write_info(f"Start Sorting Move Sequence...")
        ecs_env = ECSEnvironment(data_path=self.data_path, is_limited_count=False, 
                                 is_filter_unmovable=self.is_filter_unmovable, is_process_numa=self.is_process_numa)
        ecs_env.reset()
            ...
            while this_move_flag[target_idx]:
                ...
                for ci, i in enumerate(undo_idxes):
                    if (not move_flag[i]) and this_move_flag[i] and (simu_env.get_available_actions()[moves[i][0], moves[i][1]] > 0):
                        _, rew, _, sinfos = simu_env.step(list(moves[i]), numa_moves[i])
                        num_move += 1
                        simu_reward += rew
                ...
                if num_move <= 0:
                    ...
                    if cidx >= len(candidate_moves):
                        break
                    _, rew, _, sinfos = simu_env.step(list(candidate_moves[cidx]))
                    simu_reward += rew
                ...
                if num_move <= 0:
                    break
                # pdb.set_trace()
            if (this_move_flag[target_idx]) or (simu_reward <= 0):
                simu_env = copy_simu_env
                new_moves = []
                new_numa_moves = []
        is_solved, break_step, copy_reward_matrix = False, 0, dcp(reward_matrix)
        if len(new_moves) > 0:
            available = True
            ...
            for i, m in enumerate(new_moves):
                ...
                log_info += f"[Break Cycle by Box] SVio {break_step}: {info['vio_info']}"
                self._write_info(log_info)
                break_step += 1
                ...
                if (not info['action_available']) or ((i == len(new_moves) - 1) and (np.sum(full_rewards) <= 0)):
            ...
            if available:
                ...
                simu_env, reward_matrix = copy_origin_env, copy_reward_matrix
    ...
    def _solve_cycle_by_item(self, moves, numa_moves, move_flag, undo_idxes, target_idx, 
        ...
        def _try_break_cycle(mi, mr, break_step, reward_matrix, is_check_rew=True):
            if (simu_env.actions_available[moves[mi][0]].sum() > 0):
                if is_check_rew:
                    ...
                    if max_r < mr and (np.abs(mr - max_r) / (min(abs(mr), abs(max_r)) + 1) > 10):
                        return False, None, None, 0., None, reward_matrix
                ...
                info += f"[Break Cycle by Item] SVio {break_step}: {infos['vio_info']}"
                self._write_info(info)
                ...
                reward_matrix[:, box_update] = reward_update
                reward_matrix += simu_env._get_vio_cost_batch()
                reward_matrix[(1-available_actions).astype(bool)] = -1e99
                ...
                return True, action, infos['numa_action'], reward, item_cur_box, reward_matrix
            else:
                return False, None, None, 0., None, reward_matrix
        
        def _step(mi, break_step, reward_matrix):
        ...
        def _first_step_then_try(mi, mr, break_step, reward_matrix, is_check_rew=True):
            is_movable, action, numa_action, reward, item_cur_box, reward_matrix = _step(mi, break_step, reward_matrix)
            if (not is_movable):
                is_movable, action, numa_action, reward, item_cur_box, reward_matrix = _try_break_cycle(mi, mr, break_step, reward_matrix, is_check_rew)
            return is_movable, action, numa_action, reward, item_cur_box, reward_matrix
        ...
        while target_idx in undo_idxes:
            ...
            for idx in undo_idxes[::-1]:
                ...
                if move_flag[idx]:
                    continue
                is_movable, action, numa_action, reward, cur_box, reward_matrix = _first_step_then_try(idx, stored_rewards[idx], break_step, reward_matrix, is_check_rew=True)
                if is_movable:
                    break_step += 1
            ...
            if (not state_change) and (target_idx in undo_idxes):
                ...
                if not is_try:
                    ...
                    break
        if (target_idx in undo_idxes) or np.sum(full_rewards) <= 0.:
            simu_env, reward_matrix = copy_env, copy_reward_matrix
            break_step, step_moves, step_numa_moves, full_rewards, org_boxes = 0, [], [], [], []
            this_move_flag = np.zeros(len(moves), dtype=bool)
        # pdb.set_trace()
        is_solved, copy_move_flag = False, dcp(move_flag)
        if len(step_moves) > 0:
            available, tr = True, 0
            for i, m in enumerate(step_moves):
                _, r, _, info = self.ecs_env.step(list(m), step_numa_moves[i])
                tr += r
                if (not info['action_available'])  or ((i == len(step_moves) - 1) and (tr <= 0)):
                    for j in range(i-1, -1, -1):
            ...
            if available:
                ...
                is_solved = False
                simu_env, reward_matrix, move_flag, break_step = copy_env, copy_reward_matrix, copy_move_flag, 0
    ...
    def _solve_cycle(self, moves, numa_moves):
        ...
        for pos_m in pos_rewards:
            mi, mr = pos_m
            if reward_matrix[moves[mi][0], moves[mi][1]] <= 0:
                continue
            new_idxes = []
            ...
            if (not is_solved) and (reward_matrix[moves[mi][0], moves[mi][1]] > 0):
    ...
    def _undo_move(self, action, num_undo_move, undo_env=None, check_action_list=None, check_numa_action_list=None):
        ...
        if undo_flag:
            ...
            info += f"[Undo] SVio {num_undo_move}: {infos['vio_info']}"
            self._write_info(info)
            # pdb.set_trace()
...
if __name__ == '__main__':
    data = '0'
    data_path = f'../data/ecs_data/{data}'
    is_filter_unmovable, is_process_numa = True, True
    logger_path = None
    solution = None
    sol_save_path = f'../results/ecs_opt/opt_sols/ecs_data_addmig1.0/{data}.mst'
    is_solve_cycle, is_undo_neg, is_sorted_moves = True, True, True
    
    generator = MoveSeqGenerator(data_path, is_filter_unmovable, is_process_numa, logger_path, solution)
    generator.opt_solve_ecs(sol_save_path, is_solve_cycle, is_undo_neg, is_sorted_moves)
