...
class Agent(nn.Module):
    ...
    def get_value(self, obs_info_pm, obs_info_all_vm, obs_info_num_steps, obs_info_num_vms, graphs):
        vm_mask = torch.tensor(np.array(self.envs.call_parse('get_vm_mask', id=[1] * args.num_envs)), dtype=torch.bool, device=self.device) #
        ...
        return self.vm_net(obs_vm_embed, obs_info_num_steps, obs_pm_embed, vm_mask)[1]

    def get_action_and_value(self, envs, obs_info_pm, obs_info_all_vm, obs_info_num_steps, obs_info_num_vms,
                             graphs, vm_mask=None, pm_mask=None, selected_vm=None, selected_pm=None):
        if pm_mask is None:
        ...
        obs_vm_embed = torch.cat([obs_vm_embed, obs_info_all_vm], dim=-1)
        obs_pm_embed = torch.cat([obs_info_pm, obs_pm_embed], dim=-1)
        vm_logits, critic_score = self.vm_net(obs_vm_embed, obs_info_num_steps, obs_pm_embed, vm_mask)
    ...
    if args.track:
        wandb.init(entity="zhykoties",
                ...
                'eff_b_sz': args.accum_iter * args.num_minibatches},
                   # notes="",
                   tags=[args.model, args.gym_id, 'norm', 'GATv2'] if args.normalize else [args.model, args.gym_id, 'GATv2'],
                   save_code=True
                   )
