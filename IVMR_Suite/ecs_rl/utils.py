...
def save_checkpoint(state, global_step, checkpoint, is_best=False):
    ...
    if is_best:
        ...
        filepath = os.path.join(checkpoint, f'{global_step}.pth.tar')
...
def load_checkpoint(file_dir, restore_file, model, optimizer=None, loss=None):
    ...
    # checkpoint = os.path.join(f'/mnt/workspace/workgroup/xuwan/models/runs/{file_dir}', restore_file + '.pth.tar')
    ...
    if loss:
        ...
        return checkpoint['global_step'], model
