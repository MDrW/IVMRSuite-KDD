...
class ECSRawDenseEnvironment:
    def __init__(self, data_path, is_limited_count=True, is_filter_unmovable=True,
        ...
        self.resource_types = self.ecs_env.configs["resource_types"]
    ...
    def _init_state_scale(self):
        ...
        if self.ecs_env.configs['fixConflictOnly'] <= 0:
            cost_matrix += sparse.csr_matrix(self.ecs_env.item_assign_box_cost)
        # pdb.set_trace()
        cost_matrix = sparse.csr_matrix(cost_matrix)
        items_state = np.concatenate([
            ...
            cost_matrix.max(axis=-1).toarray().reshape(-1, 1),
            cost_matrix.min(axis=-1).toarray().reshape(-1, 1),
            np.asarray(cost_matrix.mean(axis=-1)).reshape(-1, 1),
            self.ecs_env.item_cur_box.reshape(-1, 1),
        ], axis=-1)
        boxes_state = np.concatenate([
            self.ecs_env.box_cur_state[:, :-len(self.resource_types)],
            ...
            cost_matrix.max(axis=0).toarray().reshape(-1, 1),
            cost_matrix.min(axis=0).toarray().reshape(-1, 1),
            np.asarray(cost_matrix.mean(axis=0)).reshape(-1, 1),
            self.ecs_env.box_cur_state[:, -len(self.resource_types):],
        ], axis=-1)
        
        if self.ecs_env.len_numa_res > 0:
            dense_items_state = np.concatenate([items_state, self.ecs_env.item_cur_numa], axis=-1)
        else:
            dense_items_state = items_state
        self.dense_items_map, self.dense_items_idxes = self._dense_items(dense_items_state)
        ...
        items_resources, item_state = np.array(dense_items_state)[:, 4:4+len(self.resource_types)], np.array(dense_items_state)[:, [1, 2, 3, -4, -3, -2, -1]]
        # pdb.set_trace()
        ...
        if self.is_normalize_state:
            ...
            boxes_state[:, -len(self.resource_types):] = boxes_state[:, -len(self.resource_types):] / (self.box_state_max[-len(self.resource_types):] + 1e-5)
            boxes_state[:, :-len(self.resource_types)] = (boxes_state[:, :-len(self.resource_types)] - boxes_state[:, :-len(self.resource_types)].min(axis=0)) / (boxes_state[:, :-len(self.resource_types)].max(axis=0) - boxes_state[:, :-len(self.resource_types)].min(axis=0) + 1e-5)
        else:
    ...
    def _dense_items(self, items_state, cols=None, groupby=None):
        if self.ecs_env.len_numa_res > 0:
            ncols = [f'numa_{i}' for i in range(int(self.ecs_env.len_numa_res * self.ecs_env.max_box_numa))]
        else:
            ncols = []
        if cols is None:
            cols = ['count', 'isInfinite', 'canMigrate', 'migrationCost'] + self.resource_types + ['amax', 'amin', 'amean', 'inbox'] + ncols
        if groupby is None:
            groupby=['isInfinite', 'canMigrate', 'migrationCost'] + self.resource_types + ['amax', 'amin', 'amean', 'inbox'] + ncols
        if self.is_dense_items:
    ...
    def _process_state(self, full_state, action=None):
        ...
        if action is not None:
            ...
            if self.ecs_env.configs['fixConflictOnly'] <= 0:
                this_cost_matrix = cost_matrix + sparse.csr_matrix(full_state[3])
            else:
                this_cost_matrix = cost_matrix
            boxes_state = np.concatenate([
                full_state[1][:, :-len(self.resource_types)],
                this_cost_matrix.max(axis=0).toarray().reshape(-1, 1), 
                this_cost_matrix.min(axis=0).toarray().reshape(-1, 1),
                np.asarray(this_cost_matrix.mean(axis=0)).reshape(-1, 1),
                full_state[1][:, -len(self.resource_types):],
            ], axis=1)
            box_fixed_remain_resources = self.ecs_env.box_fixed_remain_resources
            if self.is_normalize_state:
                box_fixed_remain_resources /= (self.box_state_max[-len(self.resource_types):] + 1e-5)
                boxes_state[:, -len(self.resource_types):] = boxes_state[:, -len(self.resource_types):] / (self.box_state_max[-len(self.resource_types):] + 1e-5)
                boxes_state[:, :-len(self.resource_types)] = (boxes_state[:, :-len(self.resource_types)] - boxes_state[:, :-len(self.resource_types)].min(axis=0)) / (boxes_state[:, :-len(self.resource_types)].max(axis=0) - boxes_state[:, :-len(self.resource_types)].min(axis=0) + 1e-5)
            boxes_state = np.concatenate([
                boxes_state,
            ...
            item_costs = this_cost_matrix[self.dense_items_map[i_i]]
            item_state[i_i, 4] = item_costs.max(axis=-1).max()
            item_state[i_i, 5] = item_costs.min(axis=-1).max()
            item_state[i_i, 6] = item_costs.mean(axis=-1).max()
            if self.is_normalize_state:
                item_state[i_i, 0] /= self.items_max_count
                item_state[:, 4:7] = (item_state[:, 4:7] - item_state[:, 4:7].min(axis=0)) / (item_state[:, 4:7].max(axis=0) - item_state[:, 4:7].min(axis=0) + 1e-5)
                # item_costs = (item_costs - item_costs.min(axis=0)) / ((item_costs.max(axis=0) - item_costs.min(axis=0)).toarray() + 1e-5)
            # pdb.set_trace()
            ...
            self.dense_cur_placement, self.dense_available_actions = dcp(cur_placement), dcp(available_actions)
            self.dense_items_state, self.dense_boxes_state = dcp(item_state), dcp(box_state)
        
        item_state = item_state[:, :len(item_state[0])-len(self.resource_types)+5]
        box_remain_cols = np.ones(len(box_state[0]), dtype=bool)
        box_remain_cols[7:2+len(self.resource_types)] = False
        box_remain_cols[(len(box_remain_cols)-len(self.resource_types)+5):] = False
        box_state = box_state[:, box_remain_cols]
        
        if self.is_state_merge_placement:
            items_state_in_same_box = self.dense_placement_num.T.dot(item_state) / (self.dense_placement_num.T.sum(axis=-1).reshape(-1, 1) + 1e-5)
            boxes_state_for_item = self.dense_placement_num.dot(box_state) / (self.dense_placement_num.sum(axis=-1).reshape(-1, 1) + 1e-5)
        ...
        cost_matrix = cost_matrix / (cost_matrix.max() - cost_matrix.min() + 1e-5)
        if self.ecs_env.configs['fixConflictOnly'] > 0:
            item_assign_cost = sparse.csr_matrix(np.zeros_like(item_assign_cost.toarray()))
        return item_state.copy(), box_state.copy(), sparse.csr_matrix(cur_placement.copy()), dcp(item_assign_cost), dcp(item_mutex_box), cost_matrix, sparse.csr_matrix(available_actions.copy()), dcp(self.pre_actions)
    
    def _get_cost_matrix(self):
        # vio_cost = dcp(self.ecs_env.mutex_box_vio_cost + self.ecs_env.mutex_item_vio_cost + self.ecs_env.resource_vio_cost + self.ecs_env.numa_vio_cost)
        vio_cost = -self.ecs_env._get_vio_cost_batch()
        if self.ecs_env.configs['fixConflictOnly'] <= 0:
            vio_cost += dcp(self.ecs_env.items_mix_cost)
        # if is_norm:
        #     vio_cost = vio_cost / (vio_cost.max() - vio_cost.min() + 1e-5)
        # mix_cost = dcp(self.ecs_env.items_mix_cost)
        # mix_cost = mix_cost / (mix_cost.max() - mix_cost.min() + 1e-5)
        return sparse.csr_matrix(vio_cost)
    ...
    def step(self, action, is_act_dense=False, numa_action=None):
        ...
        infos['real_reward'] += -(infos['mutex_box_vio'] + infos['mutex_item_vio'] + infos['resource_vio'] + infos['numa_vio'])
    ...
    def get_cur_state_vio_cost(self):
        return self.ecs_env.get_cur_state_vio_cost()
