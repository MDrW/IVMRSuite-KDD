...
class ECSEnvironment:
    ...
    def reset(self):
        ...
        self.box_remain_resources = self.box_infos[self.resource_types].values - self.init_placement.T.dot(self.item_infos[self.resource_types].values)
        ...
        self.box_cur_state[:, -len(self.resource_types):] = self.box_remain_resources
        ...
        self.box_fixed_remain_resources = self.box_infos[self.resource_types].values.astype(float) - fixed_placement.T.dot(self.item_infos[self.resource_types].values).astype(float)
        self.box_fixed_remain_resources[self.box_infos['isInfinite'].astype(bool).values] = self.box_infos[self.resource_types].values.max(axis=0) * 2
        # pdb.set_trace()
    ...
    def step_ignore_resource_satisfaction(self, action, numa_action=None):
        ...
        unsatisfied_boxes = np.where((self.box_cur_state[:, -len(self.resource_types):] < 0).any(axis=-1) & (self.box_cur_state[:, 1] <= 0) & (self.numa_remain_resources < 0).any(axis=-1))[0]
    ...
    def find_numa_placement(self, action, numa_action):
        ...
        if (numa_action is None) and (self.len_numa_res > 0):
            ...
            if self.box_cur_state[box_j, 1] >= 1:
                ...
                selected_numa = numa_idxes[np.argsort(((box_j_numa % item_i_numa) / item_i_numa).max(axis=-1)[numa_idxes])][:self.item_numa_num[item_i]]
    ...
    def step(self, action, numa_action=None):
        ...
        if action_available:
            ...
            costs, reward = {'migration_cost': 1e10, 'box_used_cost': 1e10, 'item_assign_cost': 1e10}, -1e99
        ...
        if self.item_cur_state[item_i, 1] <= 0:
            ...
            if (self.box_cur_state[box_k, 1] <= 0):
                ...
                if self.item_cur_state[item_i, 0] <= 1:
                    self.box_fixed_remain_resources[box_k] -= self.item_cur_state[item_i, -len(self.resource_types):]
        ...
        if self.item_cur_state[item_i, 1] <= 0:
            if (self.box_cur_state[box_j, 1] <= 0):
                ...
                if self.item_cur_state[item_i, 0] <= 0:
                    self.box_fixed_remain_resources[box_j] += self.item_cur_state[item_i, -len(self.resource_types):]
            if (self.box_cur_state[box_k, 1] <= 0):
                self.box_cur_state[box_k, -len(self.resource_types):] -= self.item_cur_state[item_i, -len(self.resource_types):]
                if (self.len_numa_res > 0) and (item_i_numa is not None):
    ...
    def is_action_available(self, action, numa_action):
        ...
        if self.item_cur_state[item_i, 0] <= 0:
            return False, f"Item {item_i} has been Moved 1 Times!"
        ...
        if (box_j != box_k) and (self.box_cur_state[box_k, 1] <= 0) and (self.box_cur_state[box_k, -len(self.resource_types):] < self.item_cur_state[item_i, -len(self.resource_types):]).any():
        ...
        # Box_j equals Box_k?
        if (box_j == box_k) and (np.array(from_numa) == np.array(to_numa)).all():
...
if __name__ == '__main__':
    start = time.time()
    # data_path = '../data/novio_finite_noii_v1/8616'
    data_path = '../data/ecs_data/1671'
    env = ECSEnvironment(data_path=data_path, is_limited_count=True, is_filter_unmovable=True, is_process_numa=True)
    env.reset()
    done = False
    total_reward = 0
    print("Item Num:", len(env.item_infos), "Box Num:", len(env.box_infos), "Create and Reset Time:", time.time()-start)
    while not done:
        s1 = time.time()
